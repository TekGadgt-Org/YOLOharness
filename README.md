...
A thin, container-first, one-shot agent: `yolo "<prompt>"` or `yolo -t <minutes> "<prompt>"`. Ordinary runs execute the provider, planner, parser, dispatch, and generated commands inside one rootless Docker container as explicit non-root UID/GID 10001:10001; rootful or unknown Docker is rejected. The host process is only a bounded launcher/supervisor; `yolo --fixture` remains an explicit offline test path. Projects must grant that numeric identity access using existing ownership/ACL policy; the launcher never chmods or chowns the project.
