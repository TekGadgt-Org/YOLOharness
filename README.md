...
Authentication is separate from a run and never starts automatically. Set a permitted `YOLO_CLIENT_ID`, then run `yolo auth login`; credentials are stored at `$XDG_CONFIG_HOME/yoloharness/credentials.json` (or `YOLO_AUTH_FILE`) outside the project with mode 0600. `yolo auth status` reports only local presence/expiry and `yolo auth logout` removes local credentials; it does not claim remote revocation. For a run, set `YOLO_RESPONSES_URL` and an explicitly selected `YOLO_MODEL`. Provider URLs must use HTTPS; HTTP is accepted only for an explicit loopback fixture with `YOLO_ALLOW_INSECURE_LOCAL=1`. Endpoint overrides are intended for local tests or an operator-established trust profile, not model input. No live OAuth, entitlement, Docker isolation, or native macOS execution has been verified in this repository.
