...
The opt-in real-Docker gate requires a locally built worker image and a Docker daemon: `docker build --pull -t yoloharness-phase1:local . && npm run test:docker`. The gate exercises the real executor and runtime bridge: typed success/failure receipts, UID and workspace artifact, read-only root, no-network route, dropped capabilities, delayed-write deadline cleanup, runtime abort-signal cleanup, an actual shipped CLI child receiving OS SIGINT, missing-image fail-closed behavior, exact container absence, and daemon-observed mount/resource/security settings. The CLI SIGINT case uses a test-only environment injection (`YOLO_CLI_SIGINT_TEST=1`) with a deterministic synthetic provider; it does not read credentials or weaken production auth endpoints. See [qa/real-docker-evidence.md](qa/real-docker-evidence.md). The default `npm test` remains offline and skips this gate.
