...
This matrix records the whole-runtime migration on the current feature branch. Synthetic HOME/XDG/auth/provider fixtures only; no live provider requests or real credentials. Source candidate is `86af2dbb4364a3f7a9e7caa409568423bce65df7`; ordered evidence-only refresh commits are `dec3425`, `a3b5b08`, `e9b9d75`, and `de3dde7`, indexed in `qa/wrc-linux-evidence-index.md`. Linux evidence below uses the observed rootless Docker daemon; the configured image must be rebuilt with `yolo setup` after runtime source changes. Setup embeds the deterministic runtime source digest in the image label `org.yoloharness.source-digest`; the real-Docker gate verifies that label before using the image.

ID       Retained evidence                                      Status (current source candidate `86af2db`; evidence-only refresh `dec3425`, `a3b5b08`, `e9b9d75`, `de3dde7`)
...
WRC-12   Started-child deadline and exact absence                  PASS scoped shipped deadline/no-late-write control (started marker, exit 124, delayed descendant write absent, exact owned runtime inventory empty); broader platform/syscall coverage remains out of scope
...
WRC-15   Environment/metadata secret exclusion                    PASS scoped shipped hostile-environment/allowlist control
...
- npm test: PASS (88 pass, 9 skipped; 97 total)
...
- `YOLO_REAL_DOCKER=1 node --test test/real-docker.test.mjs`: PASS (3 pass, 0 fail, 3 skipped; named WRC-08, WRC-09, WRC-16A, lifecycle, inspection, provenance, and cleanup controls; WRC-18 shipped setup is retained by focused `wrc-18-setup.*`)
