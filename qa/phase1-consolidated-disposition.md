...
|---|---|---|
| S3-01 complete shipped-CLI matrix absent | WRC-03..20 except 16B | Partially corrected. `npm run test:docker` now passes 2/2 from a rebuilt current-source image, and the shipped row records security flags, one `/workspace` bind, env-secret exclusions, hostile provenance, newline-cwd rejection, and exact cleanup controls. Daemon-inspection rows WRC-04..06, WRC-09, WRC-11, WRC-13..14, WRC-18..19 remain explicitly unrun. |
