...
3. Refresh race and durability: fixed in `src/auth.mjs`. Generation reread prevents duplicate rotating-token use; atomic temp-directory acquisition plus an in-lock reclaim marker prevents an acquirer from becoming owner after reclamation starts; save uses temporary-file and containing-directory fsync with errors observable and removes pre-rename temporary credentials on failure. Actual two-child-process rotation, crashed-child reclaim, deterministic replacement-owner interleaving, dead-owner reclaim, and real temporary-file/directory fsync fault regressions pass. Expired credentials are refreshed before first provider traffic, while retry refresh is limited to one actual 401.
