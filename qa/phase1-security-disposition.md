...
2. Worker receipt contract: fixed in `src/docker-executor.mjs`, `worker.mjs`, and `src/runtime.mjs`. The host requires exactly one receipt with version 1, typed closed fields, matching non-empty call_id, and success/failure invariants. Malformed, extra-key, spawned-worker failure, and local HTTP provider→runtime→DockerExecutor→shipped-worker continuation cases are regression-tested.
3. Refresh race and durability: fixed in `src/auth.mjs`. Generation reread prevents duplicate rotating-token use; atomic temp-directory acquisition plus an in-lock reclaim marker prevents an acquirer from becoming owner after reclamation starts; save uses temporary-file and containing-directory fsync with errors observable and removes pre-rename temporary credentials on failure. Actual two-child-process rotation, crashed-child reclaim, dead-owner reclaim, and deterministic replacement-owner interleaving (the replacement child signals entry into the contended-lock protocol before reclaim release) are regression-tested, along with real temporary-file/directory fsync faults. Expired credentials are refreshed before first provider traffic, while retry refresh is limited to one actual 401.
