...
- A delayed workspace write scheduled after a 100ms executor deadline never appeared after waiting beyond its delay; the exact generated container name was independently absent from `docker ps -a`.
- A synthetic provider was driven through `runOnce` and the real `DockerExecutor`; aborting its signal produced `interrupted`, the delayed write never appeared, and the exact generated container name was absent.
