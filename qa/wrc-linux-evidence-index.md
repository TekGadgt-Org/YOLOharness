...
| WRC-07 | Safe unshare nested-bind fixture plus ordinary-directory control | ENVIRONMENT-BOUNDED: setup records `uid_map` Operation not permitted under uid/euid 999; exact argv/environment/phase/output are retained in `wrc-07-nested-bind-denial.json`; ordinary control succeeds; not launcher nested-bind PASS |
...
| WRC-20 | This index plus raw daemon/image/source/row evidence | COMPLETE for finite applicable Linux aggregation: exact current identities and applicable shipped results are linked; WRC-07 remains environment-bounded and WRC-21 deferred, so no whole-matrix or release approval is claimed |
| WRC-21 | Native macOS Docker Desktop gate | DEFERRED by approved Linux-only milestone; no Linux substitution claimed |

Historical snapshots remain in `qa/wrc-finding-test-inventory.md`, `qa/cli-verification-report.md`, and `qa/run-117-docker-policy-diagnostic.md`; they are not current evidence. WRC-02 raw evidence is `wrc-02-provider-payload.txt`, `wrc-02-host-trace.jsonl`, `wrc-02-command`, `wrc-02-stdout`, `wrc-02-stderr`, `wrc-02-status`, `wrc-02-timestamps.json`, and fixed-Docker self-control `wrc-02-control-command`, `wrc-02-control-stdout`, `wrc-02-control-status`; the payload is provider-emitted SSE data observed in shipped CLI output, not the request body. No real credentials, live provider request, host trust change, privilege escalation, or unowned resource deletion was used. Post-run owned container and network inventories are empty in the retained raw capture; pre-existing daemon resources are preserved.
