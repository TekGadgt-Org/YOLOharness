import { mkdir, rename, open, readFile, chmod } from 'node:fs/promises';
...
export class AuthClient {
  constructor(config) { required(config); this.config=config; this.fetch=config.fetch ?? fetch; this.store=config.store; this.refreshing=null; }
  ...
  async refresh(credentials,{signal}={}) { if(this.refreshing) return this.refreshing; this.refreshing=(async()=>{ const token=await this.request(this.config.tokenUrl,{method:'POST',headers:{'content-type':'application/x-www-form-urlencoded',accept:'application/json'},body:new URLSearchParams({grant_type:'refresh_token',refresh_token:credentials.refreshToken,client_id:this.config.clientId})},signal); if(!token.access_token) throw new AuthError('refresh returned no access token','reauth_required'); const next={...credentials,accessToken:token.access_token}; if(token.refresh_token) next.refreshToken=token.refresh_token; if(token.expires_in) next.expiresAt=Date.now()+Number(token.expires_in)*1000; await this.store.save(next); return next; })(); try{return await this.refreshing;} finally{this.refreshing=null;} }
}
