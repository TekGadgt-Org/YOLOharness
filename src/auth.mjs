...
export class AuthClient {
  ...
  async refresh(credentials,{signal}={}) { if(this.refreshing) return this.refreshing; this.refreshing=(async()=>{ const lock=`${this.store.path}.lock`; const owner=randomUUID(); const started=Date.now(); let owned=false; try { while (!owned) { try { await mkdir(lock, { mode: 0o700 }); await writeFile(`${lock}/owner.json`, JSON.stringify({ owner, pid: process.pid })); owned=true; } catch (error) { if (error.code !== 'EEXIST' || Date.now() - started >= this.lockTimeoutMs) throw new AuthError('credential refresh lock unavailable','lock_timeout'); let stale=false; try { const info=JSON.parse(await readFile(`${lock}/owner.json`,'utf8')); if (Date.now() - (await stat(lock)).mtimeMs > this.lockTimeoutMs * 2) { try { process.kill(info.pid, 0); } catch (e) { if (e.code === 'ESRCH') stale=true; } } } catch {} if (stale) await rm(lock,{recursive:true,force:true}); await new Promise(resolve=>setTimeout(resolve,25)); } }
