import { mkdir, rename, open, readFile, chmod, rm, stat } from 'node:fs/promises';
...
export class AuthStore {
  ...
  async save(credentials) {
    ...
    try { const dir = await open(dirname(this.path), 'r'); await dir.sync(); await dir.close(); } catch { /* durability is best effort on filesystems without directory fsync */ }
    return credentials;
  }
...
export class AuthClient {
  ...
  async refresh(credentials,{signal}={}) { if(this.refreshing) return this.refreshing; this.refreshing=(async()=>{ const lock=`${this.store.path}.lock`; const started=Date.now(); let owned=false; try { while (!owned) { try { await mkdir(lock, { mode: 0o700 }); owned=true; } catch (error) { if (error.code !== 'EEXIST' || Date.now() - started >= this.lockTimeoutMs) throw new AuthError('credential refresh lock unavailable','lock_timeout'); try { const info=await stat(lock); if (Date.now()-info.mtimeMs > this.lockTimeoutMs*2) await rm(lock,{recursive:true,force:true}); } catch {} await new Promise(resolve=>setTimeout(resolve,25)); } }
      const current = await this.store.load() ?? credentials;
      const token=await this.request(this.config.tokenUrl,{method:'POST',headers:{'content-type':'application/x-www-form-urlencoded',accept:'application/json'},body:new URLSearchParams({grant_type:'refresh_token',refresh_token:current.refreshToken,client_id:this.config.clientId})},signal); if(!token.access_token) throw new AuthError('refresh returned no access token','reauth_required'); const next={...current,accessToken:token.access_token}; if(token.refresh_token) next.refreshToken=token.refresh_token; if(token.expires_in) next.expiresAt=Date.now()+Number(token.expires_in)*1000; await this.store.save(next); return next;
    } finally { if (owned) await rm(lock,{recursive:true,force:true}).catch(()=>{}); } })(); try{return await this.refreshing;} finally{this.refreshing=null;} }
}
