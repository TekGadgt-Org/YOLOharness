...
export class ConfiguredProvider {
  constructor({credentials, url, model, fetch, authClient}={}) { if(!credentials?.accessToken||!url||!model) throw new TypeError('provider requires credentials, YOLO_RESPONSES_URL, and YOLO_MODEL'); this.credentials=credentials;this.url=url;this.model=model;this.fetch=fetch;this.authClient=authClient;this.partialResult=''; }
  async next({messages,tools,signal}) {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      try {
        ...
        for await(const event of new ResponsesClient({url:this.url,model:this.model,accessToken:this.credentials.accessToken,fetch:this.fetch}).respond({input:messages,tools,signal})) { if(event.type==='text_delta') { result=(result??'')+event.delta; streamedResult=result; this.partialResult=result; } if(event.type==='tool_call') toolCall=event.call; if(event.type==='completed') { if(toolCall) return {tool_call:toolCall}; return {done:true,result:event.text??result??''}; } }
        ...
        if (streamedResult) { this.partialResult = streamedResult; error.partialResult = streamedResult; }
        throw error;
      }
    }
  }
