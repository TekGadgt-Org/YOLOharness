...
async function makeFixture(root) {
  ...
  await writeFile(proxy, `#!/usr/bin/env node\nconst cp=require('child_process'),fs=require('fs');const a=process.argv.slice(2),base=${JSON.stringify(dockerPath)},net=${JSON.stringify(network)},id=${JSON.stringify(baseId)},der=${JSON.stringify(derivative)},log=${JSON.stringify(log)};const original=[...a],r={argv:original,env:{DOCKER_HOST:process.env.DOCKER_HOST,DOCKER_CONTEXT:process.env.DOCKER_CONTEXT}};if(a[0]==='create'){for(let i=a.length-1;i>=0;i--)if(a[i]==='--group-add')a.splice(i,2);if(${JSON.stringify(true)}&&original[0]==='create'){const u=a.indexOf('--user');if(u>=0&&process.env.YOLO_TEST_ROOTFUL==='1')a[u+1]='0:0';for(let i=0;i<a.length;i++)if(a[i]==='--tmpfs')a[i+1]=a[i+1].replace(/uid=[0-9]+,gid=[0-9]+/,'uid=0,gid=0')}const n=a.indexOf('--network');if(n>=0)a[n+1]=net;if(a.includes(id))a[a.indexOf(id)]=der;r.image=a.at(-3);r.translatedArgv=[...a]}fs.appendFileSync(log,JSON.stringify(r)+'\\n');const x=cp.spawnSync(base,a,{encoding:'utf8',env:process.env,input:fs.readFileSync(0)});process.stdout.write(x.stdout||'');process.stderr.write(x.stderr||'');process.exit(x.status??1);`, { mode: 0o755 });
...
test('installed v0.1.1 CLI retains complete identity lifecycle evidence', { skip }, async () => {
  ...
  try {
    ...
    for (const mode of ['rootless', 'darwin', 'rootful']) {
      const modeEnv = { ...process.env, HOME: join(root, 'home'), XDG_CONFIG_HOME: fixture.config, XDG_DATA_HOME: fixture.data, DOCKER_CONFIG: join(root, 'docker-config'), DOCKER_HOST: fixture.endpoint, PATH: `${root}:${process.env.PATH}`, YOLO_TEST_ROOTFUL: mode === 'rootful' ? '1' : '0', NODE_OPTIONS: mode === 'darwin' ? `--require=${join(root, 'darwin.cjs')}` : undefined };
      ...
      const run = spawnSync(process.execPath, [cli, '--json', '-t', '0.1', `identity evidence ${mode}`], { cwd: fixture.workspace, env: modeEnv, encoding: 'utf8' }); assert.equal(run.status, 0, `${run.stderr}\n${run.stdout}`);
      ...
      assert.equal(records.some(r => r.argv[0] === 'info'), true); assert.equal(records.some(r => r.argv[0] === 'inspect'), true); for (const op of ['start', 'stop', 'kill', 'rm']) assert.equal(records.some(r => r.argv[0] === op), true);
      assert.equal(docker('ps', '-aq', '--no-trunc', '--filter', `name=^/${foreignName}$`).trim(), foreignId); assert.equal(docker('ps', '-aq', '--no-trunc', '--filter', `label=yoloharness.run=${create.argv[5].split('=')[1]}`).trim(), '');
...
test('installed v0.1.0 CLI retains the historical rootless-only RED', { skip }, async () => {
  ...
  try {
    ...
    assert.equal(run.status, 1); assert.match(run.stderr, /unsupported|rootless|verify/i); assert.equal(run.stdout, '');
