...
test('final runtime image exposes the documented Python/Node/Git/sh baseline under shipped restrictions', { skip }, async () => {
  ...
  try {
    ...
    const output = docker('run', '--rm', '--pull=never', '--read-only', '--cap-drop=ALL', '--security-opt', 'no-new-privileges', '--pids-limit', '128', '--memory', '512m', '--cpus', '1', '--tmpfs', '/tmp:rw,noexec,nosuid,size=64m', '--mount', `type=bind,src=${workspace},dst=/workspace,readonly=false,bind-propagation=rprivate`, '--user', '0:0', '--entrypoint', 'sh', image, '-c', command);
