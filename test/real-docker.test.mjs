...
async function fixture(t) {
  ...
  try {
    ...
    const child = spawn(process.execPath, [new URL('../src/cli.mjs', import.meta.url).pathname, '--json', `whole-runtime nonce synthetic $(touch ${hostModelCanary}) /etc/shadow`], { cwd: workspace, env, stdio: ['ignore', 'pipe', 'pipe'] });
    ...
    const secondRequest = JSON.parse(await readFile(join(capture, 'request-2.json'), 'utf8')); assert.match(secondRequest.body, /synthetic-1/);
      ...
      assert.equal(hostTrace.some(entry => entry.args.some(value => /host-model-canary|etc\/shadow|\$\(touch|hostile-image|YOLO_DOCKER_COMMAND/i.test(value))), false, 'model-derived values must not reach host operations');
      if (process.env.YOLO_EVIDENCE_DIR) {
        ...
        await writeFile(join(process.env.YOLO_EVIDENCE_DIR, 'wrc-02-provider-payload.txt'), `${request.body}\n`);
      }
    ...
    assert.match(`${pidPressure.out}${pidPressure.err}`, /(?:failed|denied|container exited|effect|128)/i);
    ...
    assert.match(`${memoryPressure.out}${memoryPressure.err}`, /(?:137|killed|container exited|effect)/i);
    ...
    if (process.env.YOLO_EVIDENCE_DIR) {
      ...
      await writeFile(join(process.env.YOLO_EVIDENCE_DIR, 'wrc-11-shipped-memory-pressure.stdout'), memoryPressure.out);
...
test('WRC-07 records safe nested-bind setup denial and ordinary nested-directory control', { skip: skip || process.env.YOLO_WRC07_ONLY !== '1' }, async () => {
  ...
  try {
    ...
    try { output = execFileSync('unshare', ['--user', '--map-root-user', '--mount', '--', 'mount', '--bind', root, nested], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] }); }
