  ...
  try {
    ...
    const secondRequest = JSON.parse(await readFile(join(capture, 'request-2.json'), 'utf8')); assert.match(secondRequest.body, /synthetic-1/);
      ...
      if (process.env.YOLO_EVIDENCE_DIR) {
        ...
        await writeFile(join(process.env.YOLO_EVIDENCE_DIR, 'wrc-02-command'), `${process.execPath} ${new URL('../src/cli.mjs', import.meta.url).pathname} --json whole-runtime nonce synthetic <provider-emitted-hostile-payload>\n`);
