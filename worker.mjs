...
async function run(call) {
  if (!validate(call)) return { version: 1, ok: false, error: 'invalid exec call' };
...
else { try { const request = JSON.parse(lines[0]); receipt = request.version !== 1 ? { version: 1, ok: false, error: 'unsupported request version' } : await run(request.call); } catch { receipt = { version: 1, ok: false, error: 'invalid request' }; } }
